...
export function fetchSubject(id) {
  return (dispatch) => {
    ...
    .then((subject) => {
      dispatch({
        type: "FETCHING_SUBJECT_SUCCESS_V4",
        subject,
      });
      ...
      .then((agg) => {
        ...
        dispatch({
          type: "FETCHING_AGGREGATIONS_SUCCESS_V4",
          aggregations,
        });
      })  //Con't ...
      ...
      .catch((err) => {
        ...
        dispatch({
          type: "FETCHING_AGGREGATIONS_ERROR_V4",
        });
      });
      //--------------------------------
    ...
    .catch((err) => {
      ...
      dispatch({
        type: "FETCHING_SUBJECT_ERROR_V4",
      });
    });
    //----------------------------------------------------------------
  };
...
export function setSubjectImageSize(imgSize) {
  return (dispatch) => {
    dispatch({
      type: "SET_SUBJECT_IMAGE_SIZE_V4",
...
export function selectRawClassification(index) {
  return (dispatch) => {
    dispatch({
      type: "SELECT_RAW_CLASSIFICATION_V4",
      index: index,
    });
  };
}
...
export function centreViewOnAggregation(index) {
  return (dispatch) => {
    dispatch({
      type: "CENTRE_VIEW_ON_AGGREGATION_V4",
      index,
    });
  };
}
